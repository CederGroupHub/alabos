...
class BaseTask(ABC):
    ...
    def update_result(self, key: str, value: Any):
        """Attach a result to the task. This will be saved in the database and can be accessed later. Subsequent calls to this function with the same key will overwrite the previous value.
    ...
    def export_result(self, key: str) -> dict:
        """Creates a reference to a result generated by this Task. This result can then be imported by another task.
            This is useful in cases where tasks are chained together. For instance, the diffraction results from a "PowderDiffraction" task could be exported, then imported by a "RietveldRefinement" analysis task.
    ...
    def import_result(
        ...
        """Imports a result from another task. This is useful in cases where tasks are chained together. For instance, the diffraction results from a "PowderDiffraction" task could be exported, then imported by a "RietveldRefinement" analysis task.

        Args:
            pointer (Union[ResultPointer, Dict[str, Any]]): Either a ResultPointer object or a dictionary with the same format as a ResultPointer.
            allow_explicit_value (bool, optional): If true, users can pass values here instead of pointers. If False, only ResultPointers are valid. Defaults to False.
        ...
        if isinstance(pointer, dict) and pointer.get("type", None) == "ResultPointer":
            ...
            if allow_explicit_value:
                ...
                raise ValueError(
                    f"Invalid pointer: {pointer}. This value was expected to be a pointer to an existing task result, but an explicit value was passed instead! If you want to allow explicit values, set allow_explicit_value=True."
                )
