...
class SamplePositionRequest(BaseModel):
    ...
    ) -> "SamplePositionRequest":  # pylint: disable=no-self-use
...
class PositionView(LabgraphMaterialView):
    ...
    def get_sample_position_status(
        ...
        If there is a Material in the position, return OCCUPIED;
        else if the sample position is locked by a task, return LOCKED;
        return EMPTY
        ...
        try:
            material = self._collection.find_one({"contents.position": position})
            sample = self._sample_view.get_by_node(material)
            return SamplePositionStatus.OCCUPIED, sample["task_id"]
        except NotFoundInDatabaseError:
            pass  # no material at this position
