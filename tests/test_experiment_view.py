...
class TestExperimentView(TestCase):
    ...
    def test_update_experiment_with_status(self):
        ...
        exp_id = self.experiment_view.create_experiment(exp_template)
        self.assertEqual(
            "PENDING", self.experiment_view.get_experiment(exp_id)["status"]
        )
        ...
        with self.assertRaises(ValueError):
            self.experiment_view.update_experiment_status(
                ObjectId(), ExperimentStatus.RUNNING
            )
    ...
    def test_get_experiments_with_status(self):
        ...
        self.assertListEqual(
            ...
            [
                exp["_id"]
                for exp in self.experiment_view.get_experiments_with_status(
        ...
        self.experiment_view.update_experiment_status(
            exp_id_1, ExperimentStatus.RUNNING
        )
        self.experiment_view.update_experiment_status(
            exp_id_2, ExperimentStatus.RUNNING
        )
        ...
        self.experiment_view.update_experiment_status(
            exp_id_1, ExperimentStatus.COMPLETED
        )
        self.assertListEqual(
            [exp_id_1],
            [
    ...
    def test_update_sample_task_id(self):
        ...
        with self.assertRaises(ValueError):
            self.experiment_view.update_sample_task_id(
                exp_id, sample_ids, [ObjectId() for _ in range(2)]
            )
        ...
        with self.assertRaises(ValueError):
            self.experiment_view.update_sample_task_id(
                exp_id, [ObjectId() for _ in range(2)], task_ids
            )
