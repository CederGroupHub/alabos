...
from simple_example.devices.furnace import Furnace
...
class Heating(BaseTask):
    ...
    def run(self):
        with self.lab_view.request_resources({Furnace: ["$/inside"]}) as (
            devices,
            sample_positions,
        ):
            furnace = devices[Furnace]
            move_to_furnace = Moving(
                sample=self.sample,
        ...
        with self.lab_view.request_resources({None: ["furnace_table"]}) as (
            devices,
            sample_positions,
        ):
            move_out_furnace = Moving(
                sample=self.sample,
                task_id=self.task_id,
